...
let RefreshTokenRepository = class RefreshTokenRepository {
    ...
    async findByUserId(userId) {
        return this.refreshTokenModel.find({ userId: new mongoose_2.Types.ObjectId(userId), isRevoked: false }).exec();
    }
    async revokeUserTokens(userId) {
        await this.refreshTokenModel.updateMany({ userId: new mongoose_2.Types.ObjectId(userId) }, { $set: { isRevoked: true } }).exec();
    }
};
