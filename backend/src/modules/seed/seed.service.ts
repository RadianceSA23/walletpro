...
export class SeedService implements OnModuleInit {
  ...
  async seedDemoData() {
    try {
      ...
      let demoUser = await this.userModel
        .findOne({ email: 'demo@expensetracker.com' })
        .exec();
      ...
      if (!demoUser) {
        ...
        await demoUser.save();
        this.logger.log(
          '✅ Updated password for Demo User (demo@expensetracker.com -> Password@123)',
        );
      }

      const adminUser = await this.userModel
        .findOne({ email: 'admin@expensetracker.com' })
        .exec();
      if (!adminUser) {
      ...
      for (const cat of requiredCategories) {
        let existing = await this.categoryModel
          .findOne({ name: cat.name, type: cat.type })
          .exec();
        if (!existing) {
      ...
      const existingIncomeCount = await this.incomeModel
        .countDocuments({ userId: demoUser._id })
        .exec();
      if (existingIncomeCount === 0) {
        ...
        this.logger.log(
          `✅ Seeded ${incomeDocs.length} realistic income records for Demo User.`,
        );
      }

      // 4. Seed Expense Records if demo user has none
      const existingExpenseCount = await this.expenseModel
        .countDocuments({ userId: demoUser._id })
        .exec();
      if (existingExpenseCount === 0) {
        ...
        this.logger.log(
          `✅ Seeded ${expenseDocs.length} realistic expense records for Demo User.`,
        );
      }
