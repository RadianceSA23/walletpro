...
export class ReportsController {
  ...
  getTopSpending(
    ...
    const userId = req.user?.id || req.user?._id;
    return this.reportsService.getTopSpendingCategories(
      userId,
      query.year,
      query.month,
      5,
  ...
  @Get('export/csv')
  @ApiOperation({
    summary: 'Export financial records as downloadable CSV spreadsheet',
  })
  ...
  async exportCSV(
    ...
    const userId = req.user?.id || req.user?._id;
    const csvData = await this.reportsService.generateCSV(
      userId,
      type || 'expenses',
      year || 2026,
      month,
    ...
    res.attachment(
      `${type || 'expenses'}-report-${year || 2026}${month ? '-' + month : ''}.csv`,
    );
